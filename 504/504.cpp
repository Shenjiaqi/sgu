...
ll hash(ll a,ll b,ll c,ll d)
{
  ...
  const ll mm=0x7FFFFFFFFFFFFFFFLL;
  ll r=(a<<7)+(b>>1);
  b=((b<<5)+(c>>2));
  r^=((c<<3)+(d>>3));
  r^=b;
  a=(a>>1)+(d<<5);
  return (r^a)&mm;
  //ll r=(( (a<<7) + (b>>1) ) ^ ((b<<6)+(c>>2)) ^ ((c<<5) + (d>>3)))+((a>>1)^b^c+d);
  //return r&mm;
}
...
int main()
{
  ...
  for(int i=0;i<h;++i)
    for(int j=0;j<w;++j)
      {
	q[i][j][0][0]=hash(131313,373737373,17171771,p[i][j]);
	for(int k=1;k<4;++k)
	  q[i][j][0][k]=q[i][j][0][0];
      }
  for(int i=0;;++i)
  ...
  for(int j=0;j<h;++j)
    for(int k=0;k<w;++k)
      {
	for(int f=max(ans[0],ans[1])+1,l=min(min(j+1,h-j),min(k+1,w-k));f<=l;)
	  {
	    int mid=(f+l)/2;
	    if(check1(j,k,mid))
	...
	for(int f=max(ans[1],ans[0]-1)+1,l=min(min(j,k),min(h-j,w-k));f<=l;)
	  {
	    int mid=(f+l)>>1;
	    if(check2(j,k,mid))
